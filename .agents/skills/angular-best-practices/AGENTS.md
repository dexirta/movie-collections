...
```html
@defer (on viewport; prefetch on idle) { <app-heavy-component /> } @defer (on interaction; prefetch
on hover) { <app-modal /> }
```
...
Use ES2024's `toSorted()`, `toReversed()`, and `toSpliced()` instead of mutating methods that require copying. Instead of `[...items].sort()`, use `items.toSorted()`. Instead of `[...items].reverse()`, use `items.toReversed()`. Instead of splice with a copy, use `items.toSpliced(index, 1)`.
...
data = this.transferState.hasKey(DATA_KEY)
  ...
  : this.http.get('/api/data').pipe(tap((d) => this.transferState.set(DATA_KEY, d)));
```
